...
bool es_tipo_valido(char tipo) {
  return (tipo == TIPO_NORMAL) || (tipo == TIPO_FUEGO) || (tipo == TIPO_AGUA) ||
         (tipo == TIPO_PLANTA) || (tipo == TIPO_ELECTRICO) ||
         (tipo == TIPO_ROCA);
}
...
void asignar_pokemon(informacion_pokemon_t *info, char nombre_pokemon[20],
  ...
  pokemon_t *aux =
      realloc(info->pokemones,
              sizeof(struct pokemon) * (long unsigned int)(info->cantidad + 1));
  if (aux != NULL) {
    info->pokemones = aux;
  }
}
...
void asignar_ataque(informacion_pokemon_t *info, char nombre_ataque[20],
  ...
  strcpy(info->pokemones[info->cantidad - 1]
             .ataques[info->pokemones[info->cantidad - 1].cantidad_ataques - 1]
             .nombre,
         nombre_ataque);
  asignar_tipo(
...
informacion_pokemon_t *pokemon_cargar_archivo(const char *path) {
  ...
  while (fgets(linea, sizeof(linea), archivo) != NULL && seguir_leyendo) {
    ...
    if (cantidad_delimitadores == 1 && cantidad_campos_invalidos == 0) {
      sscanf(linea, "%[^;];%c", nombre_pokemon, &tipo_pokemon);
      if (es_tipo_valido(tipo_pokemon)) {
        asignar_pokemon(info, nombre_pokemon, tipo_pokemon);
        cantidad_pokemones_validos++;
      } else {
        cantidad_campos_invalidos++;
      }
    } else if (cantidad_delimitadores == 2) {
      sscanf(linea, "%[^;];%c;%u", nombre_ataque, &tipo_movimiento, &poder);
      if (es_tipo_valido(tipo_movimiento)) {
        asignar_ataque(info, nombre_ataque, tipo_movimiento, poder);
      } else {
        cantidad_campos_invalidos++;
      }
    }
